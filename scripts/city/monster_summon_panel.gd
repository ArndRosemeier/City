## N-key monster summon modal: Random first, then every spawn_ready combat-table body.
...
## Ids shown under Random: combat_table spawn_ready ∩ catalogue spawnable.
static func summonable_ids() -> PackedStringArray:
	var out := PackedStringArray()
	for mid: String in CombatTableScript.spawnable_ids():
		var entry := CreatureCatalogScript.by_id(mid)
		if entry == null:
			push_error("MonsterSummonPanel: combat body '%s' missing from CreatureCatalog" % mid)
			assert(false, "MonsterSummonPanel: catalog gap")
			continue
