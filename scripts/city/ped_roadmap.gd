...
func build_from_district(
	...
	push_error(
		"PedRoadMap.build_from_district is retired — use StreetNavLayers + bind_graph()"
	)
	positions = PackedVector3Array()
	neighbors.clear()
	node_count = 0
...
func build_from_district_square(
	tool: VoxelTool,
	size_xz: int,
	...
	build_from_district(tool, size_xz, size_xz, ground_thickness, vs, stride)
