## Drivable roadmap for vehicles: asphalt / lane / crosswalk graph.
...
func build_from_district(
	...
	for z in range(1, size_z - 1, step):
		for x in range(1, size_x - 1, step):
			if not _is_drive_cell(tool, x, y, z):
				continue
...
func _is_drive_cell(tool: VoxelTool, x: int, y: int, z: int) -> bool:
	var mat := tool.get_voxel(Vector3i(x, y, z))
	if not VoxelMaterial.is_drivable_surface(mat):
		return false
	# Need clearance for vehicle body.
