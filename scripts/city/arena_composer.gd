...
## stepping blocks so the crown is reachable on foot. Shared stamp with the hill cave gate.
func _build_corner_spires() -> void:
	...
	for centre: Vector2i in centres:
		var crown_y := SpiralSpireScript.stamp(
			brush, centre, foot_y, VoxelMaterial.ARENA_SHELL, SpiralSpireScript.RISE
		)
		if crown_y < 0:
			push_error("ArenaComposer: corner spire stamp failed at %s" % centre)
			continue
		layout.corner_spires.append(Vector3i(centre.x, crown_y, centre.y))
