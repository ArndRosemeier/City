## The strip under a monster's feet: that every live body has one, that it is the size of that
## body, that it draws the pool it is bound to, and that death takes it away again.
##
...
## to be *below* the monster: a strip halfway up a giant is a different feature.
func _test_size_comes_off_the_body() -> void:
	...
	for id: String in bodies:
		...
		if bar.position.y <= 0.0:
			_fail("FAIL %s draws its bar at or under the pavement (%.3f)" % [id, bar.position.y])
			return
		## Below the body: under the middle of the hit volume, which is what "at its feet" means
		## for bodies that range from a 1.8-unit blob to a grown giant.
		if bar.position.y >= unit.hit_half_height():
			_fail(
				"FAIL %s draws its bar %.3f m up, at or above its half height %.3f m"
				% [id, bar.position.y, unit.hit_half_height()]
			)
			return
	...
	if widest / narrowest < 1.5:
		...
		return
	print(
		"size: minion %.2f m, GreenBlob %.2f m, Cactoro %.2f m wide, each under its own body"
