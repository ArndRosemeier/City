...
A bake writes shells, not rooms. `BuildingGrammar._fill_shell` records every walkable
storey it paints as a `StoreyPlate`, and `DistrictGenerator` turns those into one
`BuildingInterior` per lot — a stack of `InteriorRoom`s indexed under every district cell
the lot covers, so a foot position resolves to its building in one lookup. Elevator cabins
are reserved on each storey they serve, not just the ground one.

Everything else happens when the player walks in. `InteriorDecorator` picks the storey
under the foot and subdivides it once: `FloorPlanner` cuts the plate into rooms, corridors
and partition walls from the building's use (glass-partitioned offices around a service
core, flats off a double-loaded corridor, a shop with a back-of-house band), and
`FloorPlanPainter` paints those walls into the voxels on a per-tick budget so a large plate
cannot land in one frame. Then `RoomDecorator` furnishes one room per tick.

Two things keep the planner honest about the mass it is cutting into. A `FloorMask` built
from the actual voxels marks which columns have a floor and clear air above, so courtyard
voids, shafts and stepped setbacks are holes the plan routes around rather than rooms it
invents; a plate with too little standable area stays one open room. And `keep_clear`
rectangles (the elevator enclosure) only cancel a room when they swallow half of it — a
shaft clipping a corner just costs those columns.

Street doors get the last word over façade trim. Storefront bands, awnings and balconies
are painted outside the shell after it stands, and each of them would happily seal the
entrance it decorates, so `BuildingGrammar` clears a walk-out tunnel in front of every
doorway it recorded as the final step of a lot.

Checks: `tools/test_floor_planner.tscn` for layouts, reachability, determinism and the
budgeted painter, `tools/test_interior_decorator.tscn` for the bake-to-furnished pipeline,
`tools/test_room_decorator.gd` (run with `--script`) for the prop kits, and
`tools/test_door_access.tscn`, which walks a body out of all ~550 street doors across five
tiles and fails if one cannot reach the pavement. Looks:
`tools/shot_interior_floors.tscn` (run with `--spawn-district=0,0`).
